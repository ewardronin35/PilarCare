...
function initializeAllRecordsTable() {
    $('#all-records-table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '{{ route("doctor.medical-records.all-data") }}', // Ensure the route is correct
            type: 'GET',
            data: function (d) {
...
function viewMedicalRecord(recordId) {
    ...
    fetch(`/doctor/medical-records/${recordId}/view`) // Corrected URL
...
function downloadMedicalRecord(recordId) {
    window.location.href = `/doctor/medical-records/${recordId}/download-pdf`;
}

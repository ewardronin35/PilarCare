...
<label for="additional-info">
  ...
  <path
    ...
    @if($dentalRecord)
        <a href="{{ route('parent.dentalRecord.downloadPdf', ['id_number' => $dentalRecord->id_number]) }}" class="btn btn-primary no-spinner">Download Dental Record</a>
    @endif

    @if($nextExamination)
        <a href="{{ route('parent.dentalExam.downloadPdf', ['id_number' => $nextExamination->id_number]) }}" class="btn btn-primary no-spinner">Download Dental Examination</a>
    @else
  ...
  <script>
    ...
    var dentalRecordData = {
        personInfo: @json($personInfo),
        patientInfo: @json($patientInfo),
        lastExamination: @json($lastExamination),
        previousExaminations: @json($previousExaminations),
        toothHistory: @json($toothHistory),
        nextAppointment: @json($nextAppointment),
        nextExamination: @json($nextExamination), // Add this line

    };    
    var previousExaminations = @json($previousExaminations);
    var toothHistory = @json($toothHistory);

    @if ($teeth)
            var teeth = @json($teeth);
        @else
            var teeth = null;
        @endif
</script>

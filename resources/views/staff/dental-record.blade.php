<x-app-layout :pageTitle="'Dental Record'">
  ...
  <path
    ...
    <div class="dental-history">
        ...
        @if($dentalRecord)
        <a href="{{ route('staff.dentalRecord.pdf', ['id_number' => $dentalRecord->id_number]) }}" class="btn btn-primary no-spinner">Download Dental Record</a>
    @endif

    @if($nextExamination)
        <a href="{{ route('staff.dentalRecord.downloadPdf', ['id_number' => $nextExamination->id_number]) }}" class="btn btn-primary no-spinner">Download Dental Examination</a>
    @else
